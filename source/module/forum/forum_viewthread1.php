...
$_GET['ordertype'] = !empty($_GET['ordertype']) ? intval($_GET['ordertype']) : 1;
...
if(@in_array('forum_viewthread1', $_G['setting']['rewritestatus'])) {
	$canonical = rewriteoutput('forum_viewthread1', 1, '', $_G['tid'], 1, '', '');
} else {
	$canonical = 'forum.php?mod=viewthread1&tid='.$_G['tid'];
}
...
$lastmod = viewthread1_lastmod($_G['forum_thread']);
...
$threadtag = array();
viewthread1_updateviews($archiveid);
...
if(empty($_GET['viewpid'])) {
	...
	$multipage = multi($_G['forum_thread']['replies'] + 1, $_G['ppp'], $page, 'forum.php?mod=viewthread1&tid='.$_G['tid'].
...
if(!$maxposition && empty($postarr)) {
	...
	if(empty($_GET['viewpid'])) {
		if($_G['forum_thread']['special'] == 2) {
			$postarr = C::t('forum_post')->fetch_all_tradepost_viewthread1_by_tid($_G['tid'], $visibleallflag, $_GET['authorid'], $tpids, $_G['forum_pagebydesc'], $ordertype, $start_limit, ($_G['forum_pagebydesc'] ? $_G['forum_ppp2'] : $_G['ppp']));
		} elseif($_G['forum_thread']['special'] == 5) {
			$postarr = C::t('forum_post')->fetch_all_debatepost_viewthread1_by_tid($_G['tid'], $visibleallflag, $_GET['authorid'], $_GET['stand'], $_G['forum_pagebydesc'], $ordertype, $start_limit, ($_G['forum_pagebydesc'] ? $_G['forum_ppp2'] : $_G['ppp']));
		} else {
			$postarr = C::t('forum_post')->fetch_all_common_viewthread1_by_tid($_G['tid'], $visibleallflag, $_GET['authorid'], $_G['forum_pagebydesc'], $ordertype, $_G['forum_thread']['replies'] + 1, $start_limit, ($_G['forum_pagebydesc'] ? $_G['forum_ppp2'] : $_G['ppp']));
		}
	} else {
		$post = array();
		if($_G['forum_thread']['special'] == 2) {
...
if(!empty($isdel_post)) {
	...
	if($updatedisablepos && !$rushreply) {
		...
		dheader("Location:forum.php?mod=viewthread1&tid=$_G[tid]");
	}
...
if($postusers) {
	...
	foreach($postlist as $pid => $post) {
		...
		$postlist[$pid] = viewthread1_procpost($post, $_G['member']['lastvisit'], $ordertype, $maxposition);
	}
}
			...
			if($_G['forum_firstpid']) {
				...
				if($specialextra) {
					if(array_key_exists($specialextra, $_G['setting']['threadplugins'])) {
						...
						if(class_exists($classname) && method_exists($threadpluginclass = new $classname, 'viewthread1')) {
							$threadplughtml = $threadpluginclass->viewthread1($_G['tid']);
						}
					}
				}
			}
...
if(defined('IN_ARCHIVER')) {
	include loadarchiver('forum/viewthread1');
	exit();
}
...
if(empty($_GET['viewpid'])) {
	...
	include template('diy:forum/viewthread1'.$sufix.':'.$_G['fid']);
} else {
	$_G['setting']['admode'] = 0;
	...
	hookscriptoutput('viewthread1');
	$postcount = 0;
	if($_GET['from']) {
		include template($_GET['from'] == 'preview' ? 'forum/viewthread1_preview_node' : 'forum/viewthread1_from_node');
	} else {
		include template('forum/viewthread1_node');
	}
...
function viewthread1_updateviews($tableid) {
	global $_G;
...
function viewthread1_procpost($post, $lastvisit, $ordertype, $maxposition = 0) {
	global $_G, $rushreply;
	...
	$post['numbercard'] = viewthread1_numbercard($post);
...
function viewthread1_loadcache() {
	global $_G;
	...
	if($threadcachemark < $_G['forum']['threadcaches']) {
		...
		if(TIMESTAMP - $threadcache['filemtime'] > $_G['setting']['cachethreadlife']) {
			...
			viewthread1_updateviews($_G['forum_thread']['threadtableid']);
...
function viewthread1_lastmod(&$thread) {
	global $_G;
	if(!$thread['moderated']) {
		return array();
	}
...
function viewthread1_baseinfo($post, $extra) {
	global $_G;
	list($key, $type) = $extra;
	$v = '';
	if(substr($key, 0, 10) == 'extcredits') {
		...
		$v = $type ? lang('space', 'viewthread1_userinfo_sellercredit') : '<a href="home.php?mod=space&uid='.$post['uid'].'&do=trade&view=eccredit#buyercredit" target="_blank" class="vm"><img src="'.STATICURL.'image/traderank/seller/'.countlevel($post['buyercredit']).'.gif" /></a>';
	} elseif($key == 'eccredit_buyer') {
		$v = $type ? lang('space', 'viewthread1_userinfo_buyercredit') : '<a href="home.php?mod=space&uid='.$post['uid'].'&do=trade&view=eccredit#sellercredit" target="_blank" class="vm"><img src="'.STATICURL.'image/traderank/seller/'.countlevel($post['sellercredit']).'.gif" /></a>';
	} else {
		$v = getLinkByKey($key, $post);
		if($v !== '') {
			$v = $type ? lang('space', 'viewthread1_userinfo_'.$key) : $v;
		}
	}
	return $v;
}

function viewthread1_profile_nodeparse($param) {
...
function viewthread1_profile_node($type, $post) {
	global $_G;
	$tpid = false;
	if($post['verifyicon']) {
	...
	include_once template('forum/viewthread1_profile_node');
	foreach($code as $k => $p) {
		$p[] = $post;
		$template = str_replace($k, call_user_func('viewthread1_profile_nodeparse', $p), $template);
	}
	echo $template;
}

function viewthread1_numbercard($post) {
	global $_G;
	...
	foreach($_G['setting']['numbercard']['row'] as $key) {
		if(substr($key, 0, 10) == 'extcredits') {
			...
			$numbercard[] = array('link' => $getLink['link'], 'value' => $getLink['value'], 'lang' => lang('space', 'viewthread1_userinfo_'.$key));
		}
	}
	return $numbercard;
}
function getLinkByKey($key, $post, $returnarray = 0) {
	switch($key) {
		...
		case 'oltime': $v = array('value' => $post['oltime'].' '.lang('space', 'viewthread1_userinfo_hour'));break;
	}
	if(!$returnarray) {
		if($v['link']) {
